...
from payments.webhook_handlers.refund_handlers import (
...
class RefundHandlersTest(TestCase):
    def setUp(self):
        ...
        self.event_object_data = {
            ...
            "refunds": {
                "data": [{"id": "re_test", "status": "succeeded", "amount": 5000}]
            },
        }
        self.extracted_data = {
    ...
    @patch(
        "payments.webhook_handlers.refund_handlers.update_associated_bookings_and_payments"
    )
    ...
    def test_handle_booking_refunded_successful_full_refund(
        self,
        mock_extract_stripe_refund_data,
        mock_get_booking_from_payment,
        mock_update_associated_bookings_and_payments,
        mock_process_refund_request_entry,
        mock_send_refund_notifications,
    ):
        mock_extract_stripe_refund_data.return_value = self.extracted_data
        mock_get_booking_from_payment.return_value = (
        ...
        mock_process_refund_request_entry.assert_called_once_with(
            self.payment, self.service_booking, "service_booking", self.extracted_data
        )
    ...
    @patch("payments.webhook_handlers.refund_handlers.get_booking_from_payment")
    @patch("payments.webhook_handlers.refund_handlers.extract_stripe_refund_data")
    def test_handle_booking_refunded_refunded_amount_is_zero(
        self,
        mock_extract_stripe_refund_data,
        mock_get_booking_from_payment,
        mock_update_associated_bookings_and_payments,
    ...
    @patch("payments.webhook_handlers.refund_handlers.get_booking_from_payment")
    @patch("payments.webhook_handlers.refund_handlers.extract_stripe_refund_data")
    def test_handle_booking_refunded_invalid_stripe_object_type(
        self,
        mock_extract_stripe_refund_data,
        mock_get_booking_from_payment,
        mock_update_associated_bookings_and_payments,
    ...
    @patch(
        "payments.webhook_handlers.refund_handlers.extract_stripe_refund_data",
        side_effect=Exception("Test Exception"),
    )
    def test_handle_booking_refunded_exception_handling(
        self, mock_extract_stripe_refund_data
    ):
    ...
    def test_handle_booking_refund_updated_calls_handle_booking_refunded(
        self, mock_handle_booking_refunded
    ):
        handle_booking_refund_updated(self.payment, self.event_object_data)
        mock_handle_booking_refunded.assert_called_once_with(
            self.payment, self.event_object_data
        )

...
class WebhookEventModelTest(TestCase):
    ...
    def test_create_basic_webhook_event(self):
        ...
        event = WebhookEventFactory.create(
            stripe_event_id=stripe_event_id, event_type=event_type, payload=None
        )
    ...
    def test_stripe_event_id_uniqueness(self):
        ...
        WebhookEventFactory.create(
            stripe_event_id=stripe_event_id, event_type=event_type
        )

        with self.assertRaises(IntegrityError):
            WebhookEventFactory.create(
                stripe_event_id=stripe_event_id, event_type="charge.failed"
            )

    def test_auto_now_add_for_received_at(self):

        event = WebhookEventFactory.create(
            stripe_event_id="evt_time_test", event_type="invoice.paid"
        )
    ...
    def test_str_method(self):
        ...
        expected_str = (
            f"Event: {stripe_event_id} ({event_type}) received at {event.received_at}"
        )
        self.assertEqual(str(event), expected_str)

    def test_ordering_by_received_at(self):

        event1 = WebhookEventFactory.create(
            stripe_event_id="evt_order_1", event_type="type1"
        )
        time.sleep(0.01)
        event2 = WebhookEventFactory.create(
            stripe_event_id="evt_order_2", event_type="type2"
        )
        time.sleep(0.01)
        event3 = WebhookEventFactory.create(
            stripe_event_id="evt_order_3", event_type="type3"
        )
    ...
    def test_max_length_constraints(self):
        ...
        event_with_long_id = WebhookEvent(
            stripe_event_id=long_id, event_type="short_type"
        )
        with self.assertRaises(ValidationError):
            event_with_long_id.full_clean()

        event_with_long_type = WebhookEvent(
            stripe_event_id="evt_short_id", event_type=long_type
        )
        with self.assertRaises(ValidationError):
        ...
        valid_type = "b" * 100
        event = WebhookEventFactory.create(
            stripe_event_id=valid_id, event_type=valid_type
        )

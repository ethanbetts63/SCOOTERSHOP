...
def calculate_sales_refund_amount(booking, refund_policy_snapshot: dict, cancellation_datetime: datetime = None) -> dict:
    ...
    sales_enable_deposit_refund = refund_policy_snapshot.get('sales_enable_deposit_refund', False)
    sales_enable_deposit_refund_grace_period = refund_policy_snapshot.get('sales_enable_deposit_refund_grace_period', False)
    sales_deposit_refund_grace_period_hours = refund_policy_snapshot.get('sales_deposit_refund_grace_period_hours', 0)

    time_since_booking_creation = cancellation_datetime - booking.created_at
    time_since_booking_creation_hours = time_since_booking_creation.total_seconds() / 3600
    ...
    if sales_enable_deposit_refund:
        ...
        elif sales_enable_deposit_refund_grace_period and time_since_booking_creation_hours <= sales_deposit_refund_grace_period_hours:
            entitled_amount = total_paid_for_calculation
    ...
    entitled_amount = max(Decimal('0.00'), min(entitled_amount, total_paid_for_calculation))

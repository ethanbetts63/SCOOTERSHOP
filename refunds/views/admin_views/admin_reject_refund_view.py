...
class AdminRejectRefundView(AdminRequiredMixin, View):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Refund Request for booking '{booking_reference_for_email}' has been successfully rejected.",
            )
            ...
            if form.cleaned_data.get("send_rejection_email"):
                ...
                if not recipient_email:
                    if (
                        isinstance(customer_profile_object, ServiceProfile)
                        and customer_profile_object.user
                    ):
                        recipient_email = customer_profile_object.user.email
                    elif (
                        isinstance(customer_profile_object, SalesProfile)
                        and customer_profile_object.user
                    ):
                        recipient_email = customer_profile_object.user.email
                ...
                if recipient_email:
                    ...
                    try:
                        ...
                        messages.warning(
                            request, f"Failed to send user rejection email: {e}"
                        )
                else:
                    messages.warning(
                        request,
            ...
            try:
                ...
                messages.error(
                    request, f"Failed to send admin rejection notification email: {e}"
                )

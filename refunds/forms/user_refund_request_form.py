    ...
    reason = forms.CharField(
        widget=forms.Textarea(
            attrs={
                "rows": 4,
                "placeholder": _(
                    "Optional: Briefly explain why you are requesting a refund."
                ),
            }
        ),
        required=False,
    ...
    def clean(self):
        ...
        if booking_reference and email:
            try:
                ...
                if not booking_object and re.match(
                    r"^SBK-\w+", booking_reference, re.IGNORECASE
                ):
                    try:
                ...
                if not booking_object:
                    self.add_error(
                        "booking_reference",
                        _("No booking found with this reference number."),
                    )
                    return cleaned_data

                if (
                    not customer_profile
                    or customer_profile.email.lower() != email.lower()
                ):
                    self.add_error(
                        "email",
                        _(
                            "The email address does not match the one registered for this booking."
                        ),
                    )
                    return cleaned_data
                ...
                if not payment_object:
                    self.add_error(
                        "booking_reference",
                        _("No payment record found for this booking."),
                    )
                    return cleaned_data
                ...
                if existing_refund_requests.exists():
                    self.add_error(
                        None,
                        _("A refund request for this booking is already in progress."),
                    )
                    return cleaned_data
                ...
                self.add_error(
                    None,
                    _("Associated service profile not found for this service booking."),
                )
            except SalesProfile.DoesNotExist:
                self.add_error(
                    None,
                    _("Associated sales profile not found for this sales booking."),
                )
            except Exception as e:
                self.add_error(
                    None,
                    _(
                        f"An unexpected error occurred: {e}. Please try again or contact support."
                    ),
                )
        else:
            pass

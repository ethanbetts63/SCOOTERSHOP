...
class AdminAddEditRefundRequestViewTest(TestCase):
    ...
    def test_get_edit_refund_request_service_booking(self):
        ...
        edit_url = reverse(
            "refunds:edit_refund_request", kwargs={"pk": refund_request.pk}
        )
        ...
        self.assertIn(
            f"Edit Service Refund Request for Booking {self.service_booking.service_booking_reference}",
    ...
    def test_get_edit_refund_request_sales_booking(self):
        ...
        self.assertEqual(response.context["form"].instance, refund_request)
        self.assertIn(
            f"Edit Sales Refund Request for Booking {self.sales_booking.sales_booking_reference}",
            response.context["title"],
        )
    ...
    @patch("django.contrib.messages.success")
    def test_post_edit_valid_refund_request_service_booking(
        self, mock_messages_success
    ):
    ...
    def test_post_edit_invalid_refund_request(self, mock_messages_error):
        refund_request = RefundRequestFactory(
            service_booking=self.service_booking, payment=self.payment
        )
        edit_url = reverse(
            "refunds:edit_refund_request", kwargs={"pk": refund_request.pk}
        )

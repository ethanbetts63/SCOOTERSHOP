...
class UserRefundRequestViewTest(TestCase):
    def setUp(self):
        ...
        self.service_booking = ServiceBookingFactory(
            payment=self.payment, service_profile=self.service_profile
        )
        self.sales_booking = SalesBookingFactory(
            payment=self.payment, sales_profile=self.sales_profile
        )
    ...
    def test_get_request(self):
        ...
        self.assertIn(
            "Please enter your booking details", response.context["intro_text"]
        )

    @patch("payments.views.Refunds.user_refund_request_view.send_templated_email")
    @patch("django.contrib.messages.success")
    @patch("uuid.uuid4")
    def test_post_valid_refund_request_service_booking(
        self, mock_uuid4, mock_messages_success, mock_send_templated_email
    ):
        ...
        self.assertRedirects(
            response, reverse("refunds:user_confirmation_refund_request")
        )
        ...
        self.assertEqual(
            str(refund_request.verification_token), str(mock_uuid4.return_value)
        )
    ...
    @patch("django.contrib.messages.success")
    @patch("uuid.uuid4")
    def test_post_valid_refund_request_sales_booking(
        self, mock_uuid4, mock_messages_success, mock_send_templated_email
    ):

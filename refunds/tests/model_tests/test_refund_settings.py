...
class RefundSettingsModelTests(TestCase):
    ...
    def test_singleton_creation_raises_error(self):
        ...
        with self.assertRaises(ValidationError) as context:
            RefundSettings.objects.create(full_payment_full_refund_days=10)
    ...
    def test_default_values(self):
        ...
        self.assertEqual(settings.deposit_partial_refund_percentage, Decimal("50.00"))
    ...
    def test_days_thresholds_validation_failure_full_payment_order(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            settings.full_clean()
        self.assertIn("full_payment_full_refund_days", cm.exception.message_dict)
        ...
        with self.assertRaises(ValidationError) as cm:
            settings.full_clean()
        self.assertIn("full_payment_partial_refund_days", cm.exception.message_dict)
    ...
    def test_days_thresholds_validation_failure_deposit_order(self):
        ...
        with self.assertRaises(ValidationError) as cm:
            settings.full_clean()
        self.assertIn("deposit_full_refund_days", cm.exception.message_dict)
        ...
        with self.assertRaises(ValidationError) as cm:
            settings.full_clean()
        self.assertIn("deposit_partial_refund_days", cm.exception.message_dict)

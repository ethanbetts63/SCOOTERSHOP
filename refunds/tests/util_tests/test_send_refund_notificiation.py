...
class SendRefundNotificationsTest(TestCase):
    ...
    def test_service_booking_refund_notification_user_email(
        self, mock_send_templated_email
    ):
        service_profile = ServiceProfileFactory(
            user=self.user, email=""
        )  # Ensure email comes from user
        booking = ServiceBookingFactory(
            service_profile=service_profile, amount_paid=Decimal("100.00")
        )
        payment = PaymentFactory(service_booking=booking, amount=Decimal("100.00"))
        refund_request = RefundRequestFactory(
            payment=payment, service_booking=booking, reason="Test reason"
        )

        send_refund_notifications(
            payment, booking, "service_booking", refund_request, self.extracted_data
        )
        ...
        self.assertEqual(
            kwargs["context"]["refunded_amount"],
        ...
        self.assertIn(
            settings.SITE_BASE_URL + "/returns/",
            kwargs["context"]["refund_policy_link"],
        )

    @mock.patch("refunds.utils.send_refund_notification.send_templated_email")
    def test_sales_booking_refund_notification_user_email(
        self, mock_send_templated_email
    ):
        ...
        booking = SalesBookingFactory(
            sales_profile=sales_profile, amount_paid=Decimal("100.00")
        )
        payment = PaymentFactory(sales_booking=booking, amount=Decimal("100.00"))
        refund_request = RefundRequestFactory(
            payment=payment, sales_booking=booking, reason="Test reason"
        )

        send_refund_notifications(
            payment, booking, "sales_booking", refund_request, self.extracted_data
        )
        ...
        self.assertEqual(
            kwargs["context"]["booking_reference"], booking.sales_booking_reference
        )
        self.assertEqual(kwargs["context"]["customer_name"], sales_profile.name)

    @mock.patch("refunds.utils.send_refund_notification.send_templated_email")
    def test_service_booking_refund_notification_profile_email_no_user(
        self, mock_send_templated_email
    ):
    ...
    @mock.patch("refunds.utils.send_refund_notification.send_templated_email")
    def test_sales_booking_refund_notification_profile_email_no_user(
        self, mock_send_templated_email
    ):
    ...
    def test_admin_notification_service_booking(self, mock_send_templated_email):
        ...
        args, kwargs = mock_send_templated_email.call_args_list[
            1
        ...
        self.assertEqual(
            kwargs["context"]["stripe_refund_id"],
            self.extracted_data["stripe_refund_id"],
        )
        self.assertEqual(kwargs["context"]["payment_id"], payment.id)
        self.assertEqual(
            kwargs["context"]["payment_intent_id"], payment.stripe_payment_intent_id
        )
        self.assertEqual(
            kwargs["context"]["status"], self.extracted_data["refund_status"]
        )
        self.assertEqual(kwargs["context"]["event_type"], "charge.refunded")
        self.assertEqual(kwargs["context"]["booking_type"], "service_booking")
        self.assertEqual(kwargs["context"]["customer_name"], service_profile.name)
        self.assertEqual(
            kwargs["context"]["refund_request_reason"], refund_request.reason
        )
    ...
    def test_no_user_email_no_notification_sent(self, mock_send_templated_email):
        service_profile = ServiceProfileFactory(
            user=None, email="profile_only@example.com"
        )  # Profile has email, but no associated user
        booking = ServiceBookingFactory(
            service_profile=service_profile, amount_paid=Decimal("100.00")
        )
        payment = PaymentFactory(service_booking=booking, amount=Decimal("100.00"))
    ...
    def test_admin_initiated_refund_user_message(self, mock_send_templated_email):
        ...
        args, kwargs = mock_send_templated_email.call_args_list[0]
        self.assertEqual(
            kwargs["context"]["admin_message_from_refund"], "Admin initiated reason"
        )
    ...
    def test_refund_request_is_none(self, mock_send_templated_email):
        ...
        send_refund_notifications(
            payment, booking, "service_booking", None, self.extracted_data
        )

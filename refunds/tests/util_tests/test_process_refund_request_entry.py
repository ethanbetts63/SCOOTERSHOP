...
class ProcessRefundRequestEntryTest(TestCase):
    ...
    def test_new_refund_request_creation_service_booking(self):
        ...
        payment = PaymentFactory(service_booking=booking, amount=Decimal("100.00"), status="partially_refunded")

        refund_request = process_refund_request_entry(payment, booking, "service_booking", self.extracted_data)
        ...
        self.assertEqual(refund_request.stripe_refund_id, self.extracted_data["stripe_refund_id"])
        self.assertEqual(refund_request.amount_to_refund, self.extracted_data["refunded_amount_decimal"])
    ...
    def test_new_refund_request_creation_sales_booking(self):
        ...
        payment = PaymentFactory(sales_booking=booking, amount=Decimal("100.00"), status="refunded")

        refund_request = process_refund_request_entry(payment, booking, "sales_booking", self.extracted_data)
    ...
    def test_existing_refund_request_update_service_booking(self):
        ...
        refund_request = process_refund_request_entry(payment, booking, "service_booking", updated_extracted_data)
        ...
        self.assertEqual(refund_request.stripe_refund_id, updated_extracted_data["stripe_refund_id"])
        self.assertEqual(refund_request.amount_to_refund, updated_extracted_data["refunded_amount_decimal"])
    ...
    def test_existing_refund_request_update_sales_booking(self):
        booking = SalesBookingFactory(amount_paid=Decimal("100.00"))
        payment = PaymentFactory(sales_booking=booking, amount=Decimal("100.00"), status="partially_refunded")
        ...
        refund_request = process_refund_request_entry(payment, booking, "sales_booking", updated_extracted_data)
    ...
    def test_status_setting_refunded(self):
        booking = ServiceBookingFactory(amount_paid=Decimal("100.00"))
        payment = PaymentFactory(service_booking=booking, amount=Decimal("100.00"), status="refunded")
        refund_request = process_refund_request_entry(payment, booking, "service_booking", self.extracted_data)
        self.assertEqual(refund_request.status, "refunded")

    def test_status_setting_partially_refunded(self):
        booking = ServiceBookingFactory(amount_paid=Decimal("100.00"))
        payment = PaymentFactory(service_booking=booking, amount=Decimal("100.00"), status="succeeded") # Not 'refunded'
        refund_request = process_refund_request_entry(payment, booking, "service_booking", self.extracted_data)
        self.assertEqual(refund_request.status, "partially_refunded")
    ...
    def test_no_matching_refund_request_status(self):
        ...
        refund_request = process_refund_request_entry(payment, booking, "service_booking", new_extracted_data)

...
class SalesBookingFactory(factory.django.DjangoModelFactory):
    ...
    amount_paid = factory.Faker("pydecimal", left_digits=4, right_digits=2, positive=True)

class RefundRequestFactory(factory.django.DjangoModelFactory):
    class Meta:
        model = RefundRequest

    amount_to_refund = factory.Faker("pydecimal", left_digits=3, right_digits=2, positive=True)

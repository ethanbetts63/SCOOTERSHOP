...
class AdminRejectRefundFormTest(TestCase):
    ...
    def test_form_widgets(self):
        ...
        self.assertIn(
            "form-control", form.fields["rejection_reason"].widget.attrs["class"]
        )
        self.assertIn(
            "form-check-input",
            form.fields["send_rejection_email"].widget.attrs["class"],
        )
    ...
    def test_form_labels(self):
        ...
        self.assertEqual(
            form.fields["send_rejection_email"].label,
    ...
    def test_form_help_texts(self):
        form = AdminRejectRefundForm()
        self.assertEqual(
            form.fields["rejection_reason"].help_text,
            "Provide a clear and concise reason for rejecting this refund request.",
        )
        self.assertEqual(
            form.fields["send_rejection_email"].help_text,
            "If checked, an email containing the rejection reason will be sent to the user.",
        )

...
class GetMotorcyclesByCriteriaTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.honda_new = create_and_set_date(
            "Honda", "CBR", 2023, Decimal("15000.00"), 1000, [cls.condition_new.name]
        )
        cls.yamaha_new = create_and_set_date(
            "Yamaha", "YZF", 2022, Decimal("12000.00"), 800, [cls.condition_new.name]
        )
        cls.suzuki_new = create_and_set_date(
            "Suzuki", "GSXR", 2023, Decimal("13000.00"), 750, [cls.condition_new.name]
        )
        cls.honda_used = create_and_set_date(
            "Honda", "CRF", 2020, Decimal("8000.00"), 650, [cls.condition_used.name]
        )
        cls.kawasaki_used = create_and_set_date(
            "Kawasaki",
            "Ninja",
            2019,
            Decimal("7000.00"),
        ...
        cls.yamaha_demo = create_and_set_date(
            "Yamaha", "MT", 2021, Decimal("9500.00"), 900, [cls.condition_demo.name]
        )
        cls.ducati_demo = create_and_set_date(
            "Ducati",
            "Monster",
            2021,
            Decimal("11000.00"),
    ...
    def test_get_motorcycles_no_filters(self):
        ...
        self.assertQuerySetEqual(
            queryset, self.expected_default_order_full_dataset, ordered=True
        )
    ...
    def test_filter_by_price_range(self):
        queryset = get_motorcycles_by_criteria(
            price_min=Decimal("9000.00"), price_max=Decimal("13000.00")
        )
    ...
    def test_combined_filters(self):
        ...
        queryset_new_large_engine = get_motorcycles_by_criteria(
            condition_slug="new", engine_min_cc=800
        )
    ...
    def test_ordering_age_new_to_old(self):
        ...
        expected_sorted = sorted(
            years_and_dates, key=lambda x: (x[0], x[1]), reverse=True
        )
    ...
    def test_ordering_default_date_posted(self):
        base_test_time = datetime.datetime(
            2024, 6, 1, 0, 0, 0, tzinfo=datetime.timezone.utc
        )

        test_moto_c = MotorcycleFactory(
            brand="DefaultOrderTest", model="C", year=2019, engine_size=400
        )
        test_moto_c.date_posted = base_test_time + datetime.timedelta(seconds=0)
        test_moto_c.save()

        test_moto_a = MotorcycleFactory(
            brand="DefaultOrderTest", model="A", year=2020, engine_size=500
        )
        test_moto_a.date_posted = base_test_time + datetime.timedelta(seconds=1)
        test_moto_a.save()

        test_moto_b = MotorcycleFactory(
            brand="DefaultOrderTest", model="B", year=2021, engine_size=600
        )

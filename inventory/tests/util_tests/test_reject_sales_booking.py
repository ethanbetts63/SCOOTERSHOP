...
from inventory.tests.test_helpers.model_factories import SalesBookingFactory, MotorcycleFactory, UserFactory, PaymentFactory
...
class RejectSalesBookingTests(TestCase):
    ...
    def setUp(self):
        ...
        self.sales_booking_used_deposit = SalesBookingFactory(motorcycle=self.motorcycle_used, payment=PaymentFactory(amount=Decimal('100.00')), payment_status='deposit_paid', booking_status='pending')
        self.sales_booking_no_deposit = SalesBookingFactory(motorcycle=self.motorcycle_used, payment=None, payment_status='no_payment', booking_status='pending')
    ...
    def test_reject_sales_booking_new_motorcycle_with_refund(self, mock_create_refund, mock_send_email):
        ...
        result = reject_sales_booking(self.sales_booking_new_deposit.id, self.admin_user, form_data)
    ...
    def test_reject_sales_booking_used_motorcycle_no_refund(self, mock_send_email):
        ...
        result = reject_sales_booking(self.sales_booking_used_deposit.id, self.admin_user, form_data)
        
        self.assertTrue(result['success'])
        self.assertEqual(result['message'], 'Sales booking rejected successfully.')
        
        self.sales_booking_used_deposit.refresh_from_db()
        self.assertEqual(self.sales_booking_used_deposit.booking_status, 'declined_refunded')
    ...
    def test_reject_sales_booking_already_cancelled(self):
        ...
        result = reject_sales_booking(self.sales_booking_new_deposit.id, self.admin_user, {})
        self.assertFalse(result['success'])
    ...
    def test_reject_sales_booking_no_deposit_no_refund(self, mock_send_email):
        ...
        result = reject_sales_booking(self.sales_booking_no_deposit.id, self.admin_user, form_data)

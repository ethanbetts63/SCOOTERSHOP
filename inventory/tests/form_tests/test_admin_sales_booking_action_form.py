...
class SalesBookingActionFormTest(TestCase):
    ...
    def test_reject_action_refund_amount_exceeds_paid_amount(self):
        ...
        self.assertEqual(
            form.errors["refund_amount"],
            [
                f"Refund amount cannot exceed the amount paid ({self.sales_booking.payment.amount:.2f} {self.sales_booking.payment.currency})."
            ],
        )
    ...
    def test_reject_action_zero_amount_paid_for_booking(self):
        ...
        self.assertEqual(
            form.errors["refund_amount"],
            [
                f"Refund amount cannot exceed the amount paid ({self.sales_booking_zero_payment.payment.amount:.2f} {self.sales_booking_zero_payment.payment.currency})."
            ],
        )

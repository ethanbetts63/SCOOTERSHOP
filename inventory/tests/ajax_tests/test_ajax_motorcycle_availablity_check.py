...
class AjaxMotorcycleAvailabilityCheckTest(TestCase):
    ...
    def test_motorcycle_is_available(self):
        payload = {
            'temp_booking_uuid': str(self.available_temp_booking.session_uuid)
        }
    ...
    def test_motorcycle_is_not_available(self):
        payload = {
            'temp_booking_uuid': str(self.unavailable_temp_booking.session_uuid)
        }
        response = self.client.post(
            reverse('inventory:ajax_check_motorcycle_availability'),
            json.dumps(payload),
        ...
        self.assertEqual(data['message'], 'Sorry, this motorcycle has just been reserved or sold.')

    def test_temp_booking_not_found(self):
        non_existent_uuid = uuid.uuid4()
        payload = {
            'temp_booking_uuid': str(non_existent_uuid)
        }
        response = self.client.post(
            reverse('inventory:ajax_check_motorcycle_availability'),
            json.dumps(payload),
    ...
    def test_associated_motorcycle_not_found(self):
        payload = {
            'temp_booking_uuid': str(self.temp_booking_with_missing_motorcycle.session_uuid)
        }
        response = self.client.post(
            reverse('inventory:ajax_check_motorcycle_availability'),
            json.dumps(payload),

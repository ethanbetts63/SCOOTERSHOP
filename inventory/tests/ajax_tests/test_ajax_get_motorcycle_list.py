...
class AjaxGetMotorcycleListTest(TestCase):
    ...
    def test_filter_by_brand(self):
        response = self.client.get(reverse('inventory:ajax-get-motorcycle-list'), {'brand': 'Honda'})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
    ...
    def test_filter_by_year(self):
        response = self.client.get(reverse('inventory:ajax-get-motorcycle-list'), {'year_min': 2022, 'year_max': 2023})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        self.assertEqual(len(data['motorcycles']), 2)
    ...
    def test_filter_by_price(self):
        response = self.client.get(reverse('inventory:ajax-get-motorcycle-list'), {'price_min': 9000, 'price_max': 12000})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        self.assertEqual(len(data['motorcycles']), 2)
    ...
    def test_filter_by_engine_size(self):
        response = self.client.get(reverse('inventory:ajax-get-motorcycle-list'), {'engine_min_cc': 950, 'engine_max_cc': 1200})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        self.assertEqual(len(data['motorcycles']), 2)
    ...
    def test_sort_by_price_low_to_high(self):
        response = self.client.get(reverse('inventory:ajax-get-motorcycle-list'), {'order': 'price_low_to_high'})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        prices = [float(m['price']) for m in data['motorcycles']]
    ...
    def test_sort_by_price_high_to_low(self):
        response = self.client.get(reverse('inventory:ajax-get-motorcycle-list'), {'order': 'price_high_to_low'})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        prices = [float(m['price']) for m in data['motorcycles']]
    ...
    def test_sort_by_year_new_to_old(self):
        response = self.client.get(reverse('inventory:ajax-get-motorcycle-list'), {'order': 'age_new_to_old'})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        years = [m['year'] for m in data['motorcycles']]
    ...
    def test_sort_by_year_old_to_new(self):
        response = self.client.get(reverse('inventory:ajax-get-motorcycle-list'), {'order': 'age_old_to_new'})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        years = [m['year'] for m in data['motorcycles']]
    ...
    def test_pagination(self):
        ...
        response = self.client.get(reverse('inventory:ajax-get-motorcycle-list'), {'page': 1})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        self.assertEqual(len(data['motorcycles']), 9)
        self.assertTrue(data['page_obj']['has_next'])

        response = self.client.get(reverse('inventory:ajax-get-motorcycle-list'), {'page': 2})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        self.assertEqual(len(data['motorcycles']), 5) # 4 from setUpTestData + 10 here = 14 total. 9 on page 1, 5 on page 2

...
class InitiateBookingProcessViewTest(TestCase):
    ...
    def test_post_initiate_booking_process_success_with_deposit(self):
        response = self.client.post(
            reverse("inventory:initiate_booking", kwargs={"pk": self.motorcycle.pk}),
            data={"deposit_required_for_flow": "true"},
        )
    ...
    def test_post_initiate_booking_process_success_without_deposit(self):
        response = self.client.post(
            reverse("inventory:initiate_booking", kwargs={"pk": self.motorcycle.pk}),
            data={"deposit_required_for_flow": "false"},
            follow=True,
        )
    ...
    def test_post_initiate_booking_process_motorcycle_not_available(self):
        ...
        response = self.client.post(
            reverse(
                "inventory:initiate_booking", kwargs={"pk": not_for_sale_motorcycle.pk}
            )
        )
        ...
        self.assertIn(
            "is currently reserved or sold and cannot be booked at this time.",
            str(messages[0]),
        )
    ...
    def test_post_initiate_booking_process_motorcycle_not_found(self):
        response = self.client.post(
            reverse("inventory:initiate_booking", kwargs={"pk": 999})
        )

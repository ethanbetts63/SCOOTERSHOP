...
class SalesEnquiryViewTest(TestCase):
    ...
    def test_get_sales_enquiry_view(self):
        response = self.client.get(
            reverse(
                "inventory:sales_enquiry", kwargs={"motorcycle_id": self.motorcycle.pk}
            )
        )
        ...
        self.assertEqual(
            response.context["form"].initial["motorcycle"], self.motorcycle
        )
    ...
    def test_post_sales_enquiry_view_success(self, mock_send_email):
        ...
        self.assertRedirects(
            response,
            reverse("inventory:motorcycle-detail", kwargs={"pk": self.motorcycle.pk}),
        )

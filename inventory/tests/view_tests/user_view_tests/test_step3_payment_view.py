...
class Step3PaymentViewTest(TestCase):
    ...
    def test_dispatch_no_motorcycle_redirects(self):
        ...
        messages = list(get_messages(response.wsgi_request))
        self.assertTrue(
            any("Please select a motorcycle first." in str(m) for m in messages)
        )
    ...
    def test_dispatch_no_sales_profile_redirects(self):
        ...
        self.assertTrue(
            any(
                "Please provide your contact details first." in str(m) for m in messages
            )
        )
    ...
    def test_dispatch_deposit_not_required_redirects_to_confirmation(self):
        ...
        redirect_url = (
            ...
            + f"?payment_intent_id={self.temp_booking.stripe_payment_intent_id}"
        )
        self.assertRedirects(response, redirect_url, fetch_redirect_response=False)
        messages = list(get_messages(response.wsgi_request))
        self.assertTrue(
            any(
                "Payment is not required for this type of booking." in str(m)
                for m in messages
            )
        )
    ...
    def test_get_creates_new_intent_and_payment_obj(self, mock_create_update):
        ...
        self.assertEqual(
            response.context["amount"], self.inventory_settings.deposit_amount
        )
    ...
    def test_post_payment_succeeded(self, mock_retrieve):
        ...
        self.assertTrue(
            json_response["redirect_url"].startswith(
                reverse("inventory:step4_confirmation")
            )
        )

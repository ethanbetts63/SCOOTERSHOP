...
class SalesfaqDeleteViewTest(TestCase):
    ...
    def test_sales_faq_delete_post_success(self):
        ...
        response = self.client.post(
            reverse("inventory:sales_faq_delete", kwargs={"pk": self.sales_faq.pk})
        )
        ...
        self.assertIn(
            "The faq 'Test FAQ Question...' was deleted successfully.", str(messages[0])
        )

    def test_sales_faq_delete_post_nonexistent(self):
        initial_count = Salesfaq.objects.count()
        response = self.client.post(
            reverse("inventory:sales_faq_delete", kwargs={"pk": 999})
        )

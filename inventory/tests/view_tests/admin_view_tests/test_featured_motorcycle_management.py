...
class FeaturedMotorcycleManagementViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.featured_new1 = FeaturedMotorcycleFactory(motorcycle=self.new_motorcycle1, category='new', order=1)
        self.featured_new2 = FeaturedMotorcycleFactory(motorcycle=self.new_motorcycle2, category='new', order=2)
        self.featured_used1 = FeaturedMotorcycleFactory(motorcycle=self.used_motorcycle1, category='used', order=1)
        self.featured_used2 = FeaturedMotorcycleFactory(motorcycle=self.used_motorcycle2, category='used', order=2)
    ...
    def test_featured_motorcycle_management_view_get(self):
        ...
        self.assertTemplateUsed(response, 'inventory/admin_featured_motorcycle_management.html')
    ...
    def test_admin_required_mixin(self):
        ...
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('inventory:featured_motorcycles'))
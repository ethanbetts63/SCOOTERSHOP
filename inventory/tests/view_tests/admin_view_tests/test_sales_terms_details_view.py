...
class SalesTermsDetailsViewTest(TestCase):
    ...
    def test_sales_terms_details_view_nonexistent(self):
        response = self.client.get(reverse('inventory:terms_and_conditions_detail', kwargs={'pk': 999}))
    ...
    def test_admin_required_mixin(self):
        ...
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('inventory:terms_and_conditions_detail', kwargs={'pk': self.sales_terms.pk}))
...
class SalesfaqManagementViewTest(TestCase):
    ...
    def test_sales_faq_management_view_pagination(self):
        response = self.client.get(reverse('inventory:sales_faq_management') + '?page=2')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'inventory/admin_sales_faq_management.html')
        self.assertIn('sales_faqs', response.context)
        self.assertEqual(len(response.context['sales_faqs']), 6) # 20 general + 1 step1 = 21 total. 21 - 15 = 6 on second page
    ...
    def test_admin_required_mixin(self):
        ...
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('inventory:sales_faq_management'))
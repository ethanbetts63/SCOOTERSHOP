...
class SalesProfileDetailsViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.sales_profile = SalesProfileFactory(user=cls.user, name="John Doe's Profile")
        cls.url = reverse('inventory:sales_profile_details', kwargs={'pk': cls.sales_profile.pk})
    ...
    def test_context_data(self):
        ...
        self.assertEqual(response.context['page_title'], f"Sales Profile Details: {self.sales_profile.name}")

    def test_404_for_non_existent_sales_profile(self):
        self.client.login(username='adminuser', password='adminpass123')
        non_existent_url = reverse('inventory:sales_profile_details', kwargs={'pk': 99999})

...
# from ...test_helpers.model_factories import BlockedSalesDateFactory, UserFactory
...
#         cls.blocked_date_to_delete = BlockedSalesDateFactory(description='Holiday Period')
#         cls.delete_url = reverse('inventory:admin_blocked_sales_date_delete', kwargs={'pk': cls.blocked_date_to_delete.pk})

#     def test_delete_blocked_sales_date_as_admin_success(self):
#         self.client.login(username='admin', password='adminpassword')
#         self.assertTrue(BlockedSalesDate.objects.filter(pk=self.blocked_date_to_delete.pk).exists())
#         initial_count = BlockedSalesDate.objects.count()

#         response = self.client.post(self.delete_url, follow=True)

#         self.assertRedirects(response, reverse('inventory:blocked_sales_date_management'))
#         self.assertEqual(BlockedSalesDate.objects.count(), initial_count - 1)
#         self.assertFalse(BlockedSalesDate.objects.filter(pk=self.blocked_date_to_delete.pk).exists())

#         messages_list = list(messages.get_messages(response.wsgi_request))
#         self.assertEqual(len(messages_list), 1)
#         self.assertEqual(str(messages_list[0]), f'Blocked sales date {self.blocked_date_to_delete} deleted successfully!')

#     def test_delete_blocked_sales_date_as_admin_not_found(self):
#         self.client.login(username='admin', password='adminpassword')
#         non_existent_pk = self.blocked_date_to_delete.pk + 999
#         url = reverse('inventory:admin_blocked_sales_date_delete', kwargs={'pk': non_existent_pk})

...
class SalesfaqCreateUpdateViewTest(TestCase):
    ...
    def test_create_sales_faq_get(self):
        ...
        self.assertTemplateUsed(response, 'inventory/admin_sales_faq_create_update.html')
    ...
    def test_update_sales_faq_get(self):
        faq = SalesfaqFactory()
        response = self.client.get(reverse('inventory:sales_faq_update', kwargs={'pk': faq.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'inventory/admin_sales_faq_create_update.html')
    ...
    def test_update_sales_faq_post_success(self):
        ...
        response = self.client.post(reverse('inventory:sales_faq_update', kwargs={'pk': faq.pk}), data=data)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('inventory:sales_faq_management'))
        faq.refresh_from_db()
    ...
    def test_admin_required_mixin(self):
        ...
        response = self.client.get(reverse('inventory:sales_faq_create'))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('inventory:sales_faq_create'))
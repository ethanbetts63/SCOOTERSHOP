...
# from ...test_helpers.model_factories import SalesProfileFactory, UserFactory
...
#         cls.sales_profile_to_delete = SalesProfileFactory(name='Profile To Delete', email='delete@example.com')
#         cls.delete_url = reverse('inventory:admin_sales_profile_delete', kwargs={'pk': cls.sales_profile_to_delete.pk})

#     def test_delete_sales_profile_as_admin_success(self):
#         self.client.login(username='admin', password='adminpassword')
#         self.assertTrue(SalesProfile.objects.filter(pk=self.sales_profile_to_delete.pk).exists())
...
#         self.assertFalse(SalesProfile.objects.filter(pk=self.sales_profile_to_delete.pk).exists())

#         messages_list = list(messages.get_messages(response.wsgi_request))
#         self.assertEqual(len(messages_list), 1)
#         self.assertEqual(str(messages_list[0]), f'Sales profile for {self.sales_profile_to_delete.name} deleted successfully!')

#     def test_delete_sales_profile_as_admin_not_found(self):
#         self.client.login(username='admin', password='adminpassword')
#         non_existent_pk = self.sales_profile_to_delete.pk + 999
#         url = reverse('inventory:admin_sales_profile_delete', kwargs={'pk': non_existent_pk})

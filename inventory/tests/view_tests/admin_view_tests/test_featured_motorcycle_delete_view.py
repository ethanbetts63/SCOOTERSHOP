...
from inventory.tests.test_helpers.model_factories import FeaturedMotorcycleFactory, UserFactory
...
class FeaturedMotorcycleDeleteViewTest(TestCase):
    ...
    def test_delete_featured_motorcycle_post(self):
        ...
        response = self.client.post(reverse('inventory:delete_featured_motorcycle', kwargs={'pk': self.featured_motorcycle.pk}))
    ...
    def test_delete_nonexistent_featured_motorcycle(self):
        initial_count = FeaturedMotorcycle.objects.count()
        response = self.client.post(reverse('inventory:delete_featured_motorcycle', kwargs={'pk': 999}))
    ...
    def test_admin_required_mixin(self):
        ...
        response = self.client.post(reverse('inventory:delete_featured_motorcycle', kwargs={'pk': self.featured_motorcycle.pk}))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('inventory:delete_featured_motorcycle', kwargs={'pk': self.featured_motorcycle.pk}))
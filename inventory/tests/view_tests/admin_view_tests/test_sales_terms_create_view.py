...
from inventory.tests.test_helpers.model_factories import UserFactory, SalesTermsFactory
...
class SalesTermsCreateViewTest(TestCase):
    ...
    def test_create_sales_terms_post_success(self):
        ...
        response = self.client.post(reverse('inventory:terms_and_conditions_create'), data=data)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('inventory:terms_and_conditions_management'))
        ...
        self.assertIn(f'New Terms & Conditions Version {new_terms.version_number} created successfully and set as active.', str(messages[0]))
    ...
    def test_admin_required_mixin(self):
        ...
        response = self.client.get(reverse('inventory:terms_and_conditions_create'))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('inventory:terms_and_conditions_create'))
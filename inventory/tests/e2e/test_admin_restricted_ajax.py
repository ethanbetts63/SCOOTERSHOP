...
class InventoryAdminAjaxPermissionsTestCase(TestCase):
    ...
    def test_admin_ajax_endpoints_permissions(self):
        self._test_ajax_permissions("inventory:admin_api_search_motorcycles", data={'query': 'test'})
        self._test_ajax_permissions("inventory:admin_api_get_motorcycle_details", kwargs={'pk': self.motorcycle.pk})
        self._test_ajax_permissions("inventory:admin_api_search_sales_profiles", data={'query': 'test'})
        self._test_ajax_permissions("inventory:admin_api_get_sales_profile_details", kwargs={'pk': self.sales_profile.pk})
        self._test_ajax_permissions("inventory:admin_api_sales_booking_precheck", method='post', data={})
        self._test_ajax_permissions("inventory:get_sales_bookings_json")
        self._test_ajax_permissions("inventory:admin_api_search_sales_bookings", data={'query': 'test'})

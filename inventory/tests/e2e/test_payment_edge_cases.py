...
class TestPaymentEdgeCases(TestCase):
    ...
    def test_failed_payment_does_not_reserve_motorcycle(self, mock_stripe_confirm):
        mock_stripe_confirm.side_effect = stripe.error.CardError(
            "Your card was declined.", param="card_error", code="card_declined"
        )
        ...
        self.assertEqual(
            SalesBooking.objects.count(),
            0,
            "No SalesBooking should be created for a failed payment.",
        )
        ...
        self.assertTrue(
            TempSalesBooking.objects.filter(session_uuid=temp_booking_uuid).exists()
        )

        payment_obj.refresh_from_db()
        self.assertNotEqual(
            payment_obj.status, "succeeded", "Payment status should not be 'succeeded'."
        )
        self.assertNotEqual(
            payment_obj.status,
            "failed",
            "Payment status should not be 'failed' unless explicitly handled.",
        )

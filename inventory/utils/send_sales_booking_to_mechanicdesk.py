...
def send_sales_booking_to_mechanicdesk(sales_booking_instance):
    ...
    customer_last_name = (
        " ".join(sales_profile.name.split(" ")[1:]) if " " in sales_profile.name else ""
    )
    ...
    if (
        ...
        and sales_booking_instance.appointment_time
    ):
        ...
        naive_datetime = datetime.datetime.combine(
            sales_booking_instance.appointment_date, datetime.time(9, 0)
        )
    ...
    notes_content += (
        f"Booking Status: {sales_booking_instance.get_booking_status_display()}\n"
    )
    ...
    if sales_booking_instance.appointment_date:
        local_time_str = (
            sales_booking_instance.appointment_time.strftime("%I:%M %p")
    ...
    payload = {
        ...
        "street_line": (
            sales_profile.address_line_1 if sales_profile.address_line_1 else ""
        ),

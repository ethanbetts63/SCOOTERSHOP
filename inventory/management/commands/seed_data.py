...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--count',
            type=int,
            help='Number of motorcycles to create.',
            default=15
        )
        parser.add_argument(
            '--no-clear',
    ...
    def _seed_conditions(self):
        ...
        for name in ['New', 'Used', 'Demo']:
            obj, created = MotorcycleCondition.objects.get_or_create(name=name.lower(), defaults={'display_name': name})
            if created:
                self.stdout.write(self.style.SUCCESS(f"  - Created '{obj.display_name}' condition."))
    ...
    def _seed_motorcycles(self, clear=True, count=15):
        ...
        if clear:
            ...
            self.stdout.write(self.style.WARNING(f"  - Cleared {deleted_count} existing motorcycles."))
        ...
        for i in range(count):
            ...
            is_used = random.choice([True, True, False]) # Make it more likely to be used
        ...
        self.stdout.write(self.style.SUCCESS(f"  - Successfully created {created_count} new motorcycles."))
    ...
    def _seed_featured_motorcycles(self, clear=True):
        ...
        if clear:
            ...
            self.stdout.write(self.style.WARNING(f"  - Cleared {deleted_count} existing featured motorcycles."))
    ...
    def _create_featured_set(self, category, count):
        ...
        self.stdout.write(f"  - Creating up to {count} featured '{category}' motorcycles...")

        condition_filter = Q(condition=category) | Q(conditions__name__iexact=category)
        existing_featured_ids = FeaturedMotorcycle.objects.values_list('motorcycle_id', flat=True)
        
        eligible_bikes = Motorcycle.objects.filter(condition_filter).exclude(pk__in=existing_featured_ids).distinct()

        if eligible_bikes.count() < count:
            count = eligible_bikes.count()
        
        if count == 0:
            self.stdout.write(self.style.WARNING(f"    - No available '{category}' motorcycles to feature."))
            return
        ...
        self.stdout.write(self.style.SUCCESS(f"    - Successfully created {created_count} new '{category}' featured instances."))

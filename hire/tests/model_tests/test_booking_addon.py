...
class BookingAddOnModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.available_addon = create_addon(name="Available AddOn", daily_cost=Decimal('10.00'), is_available=True)
        cls.unavailable_addon = create_addon(name="Unavailable AddOn", daily_cost=Decimal('20.00'), is_available=False)
    ...
    def test_clean_booked_addon_price_mismatch_raises_error(self):
        ...
        self.assertEqual(
            ...
            f"Booked add-on price must match the current price of the add-on ({self.available_addon.daily_cost})."
        )
    ...
    def test_clean_valid_booking_addon_passes(self):
        ...
        booking_addon = create_booking_addon(
            ...
            addon=self.available_addon,
            quantity=1,
            booked_addon_price=self.available_addon.daily_cost # Match
        )
        try:
            booking_addon.clean()

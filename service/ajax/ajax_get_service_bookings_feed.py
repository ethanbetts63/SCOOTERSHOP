...
def get_service_bookings_json_ajax(request):
    ...
    bookings_query = ServiceBooking.objects.select_related(
        "service_type", "service_profile__user", "customer_motorcycle"
    )
    if start_date and end_date:
        bookings_query = bookings_query.filter(
            dropoff_date__range=[start_date, end_date - timedelta(days=1)]
        )
    ...
    for booking in bookings:
        ...
        if booking.service_profile:
            if (
                booking.service_profile.user
                and booking.service_profile.user.get_full_name()
            ):
        ...
        events.append(
            {
                ...
                "extendedProps": {
                    ...
                    "service_type": (
                        booking.service_type.name if booking.service_type else "Service"
                    ),
    ...
    for blocked_date in blocked_dates:
        blocked_event_end_date_for_fc = (
            blocked_date.end_date + timedelta(days=1)
        ).isoformat()

    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            temp_booking = get_object_or_404(
                TempServiceBooking, session_uuid=temp_booking_uuid
            )
        ...
        if not temp_booking.service_profile:
            messages.error(
                request, "Please provide your contact details first (Step 4)."
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        for disabled_entry in disabled_dates:
            if isinstance(disabled_entry, str) and disabled_entry == str(date_to_check):
                ...
                isinstance(disabled_entry, dict)
                and "from" in disabled_entry
                and "to" in disabled_entry
            ):
                start_range = datetime.datetime.strptime(
                    disabled_entry["from"], "%Y-%m-%d"
                ).date()
                end_range = datetime.datetime.strptime(
                    disabled_entry["to"], "%Y-%m-%d"
                ).date()
        ...
        if is_date_now_disabled:
            messages.error(
                request,
                f"The selected service date ({date_to_check.strftime('%Y-%m-%d')}) is no longer available. Please choose another date.",
            )
        ...
        if payment_method == "online_full":
            ...
            messages.error(
                request,
                "No valid payment option selected. Please choose a payment method.",
            )
        ...
        try:
            if payment_obj and payment_obj.stripe_payment_intent_id:
                try:
                    ...
                    if (
                        amount_changed or currency_changed
                    ) and is_modifiable_or_in_progress:
                        intent = stripe.PaymentIntent.modify(
                            ...
                            metadata={
                                ...
                                "service_profile_id": (
                                    str(service_customer_profile.id)
                                    if service_customer_profile
                                    else "guest"
                                ),
            ...
            if not intent:
                ...
                if payment_obj:
                    ...
                    if (
                        ...
                        and not payment_obj.service_customer_profile
                    ):
                ...
                return JsonResponse(
                    {
                        ...
                        "redirect_url": reverse("service:service_book_step7")

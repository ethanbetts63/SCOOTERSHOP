...
class MechanicDeskIntegrationTests(TestCase):
    ...
    def test_send_booking_to_mechanicdesk_success(self, mock_post):
        ...
        self.assertEqual(
            args[0], "https://www.mechanicdesk.com.au/booking_requests/create_booking"
        )
        ...
        self.assertEqual(
            sent_data["registration_number"], self.customer_motorcycle.rego
        )
        self.assertEqual(
            sent_data["transmission"], self.customer_motorcycle.transmission
        )
        ...
        expected_dropoff_datetime_mechdesk = datetime.datetime.combine(
            self.service_booking.service_date, self.service_booking.dropoff_time
        ).strftime("%d/%m/%Y %H:%M")
        ...
        expected_pickup_datetime = datetime.datetime.combine(
            self.service_booking.estimated_pickup_date, datetime.time(17, 0)
        ).strftime("%d/%m/%Y %H:%M")
    ...
    def test_send_booking_to_mechanicdesk_no_customer_motorcycle(self, mock_post):
        ...
        expected_pickup_datetime_no_moto = datetime.datetime.combine(
            self.service_booking_no_moto.estimated_pickup_date, datetime.time(17, 0)
        ).strftime("%d/%m/%Y %H:%M")

...
from service.tests.test_helpers.model_factories import ServiceBookingFactory, UserFactory, PaymentFactory, SiteSettingsFactory
...
class RejectServiceBookingTests(TestCase):
    ...
    def setUp(self):
        ...
        # Ensure each service booking gets a unique payment instance
        self.service_booking_deposit = ServiceBookingFactory(payment=PaymentFactory(amount=Decimal('100.00')), payment_status='deposit_paid', booking_status='pending')
        self.service_booking_no_deposit = ServiceBookingFactory(payment=None, payment_status='unpaid', booking_status='pending')
        SiteSettingsFactory()

    @patch('refunds.utils.create_refund_request.create_refund_request')
    @patch('mailer.utils.send_templated_email')
    def test_reject_service_booking_with_refund(self, mock_send_email, mock_create_refund):
        ...
        result = reject_service_booking(self.service_booking_deposit.id, self.admin_user, form_data)
    ...
    def test_reject_service_booking_no_refund(self, mock_send_email):
        ...
        self.service_booking_deposit.refresh_from_db()
        self.assertEqual(self.service_booking_deposit.booking_status, 'DECLINED_REFUNDED')
    ...
    def test_reject_service_booking_already_cancelled(self):
        ...
        result = reject_service_booking(self.service_booking_deposit.id, self.admin_user, {})
        self.assertFalse(result['success'])
        self.assertEqual(result['message'], 'Booking already cancelled or declined.')

    @patch('refunds.utils.create_refund_request.create_refund_request')
    def test_reject_service_booking_with_refund_missing_amount(self, mock_create_refund):
        form_data = {
            'message': 'Test rejection message',
            'send_notification': True,
            'initiate_refund': True,
    ...
    def test_reject_service_booking_no_deposit_no_refund(self, mock_send_email):
        ...
        result = reject_service_booking(self.service_booking_no_deposit.id, self.admin_user, form_data)
    ...
    @patch('mailer.utils.send_templated_email')
    @patch('refunds.utils.create_refund_request.create_refund_request')
    def test_reject_service_booking_no_notification(self, mock_create_refund, mock_send_email):
        form_data = {
            'message': 'Test rejection message',
            'send_notification': False,
            'initiate_refund': False,
        }
        result = reject_service_booking(self.service_booking_deposit.id, self.admin_user, form_data, send_notification=False)

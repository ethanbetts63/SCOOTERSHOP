...
class TestLoggedInDepositPaymentFlow(TestCase):
    ...
    def setUp(self):
        ...
        self.service_type = ServiceTypeFactory(
            name="Account Deposit Service", base_price=Decimal("600.00"), is_active=True
        )
        ...
        self.service_profile = ServiceProfileFactory(
            user=self.user, name=user_name, email="acc.deposit@user.com", country="AU"
        )
        self.motorcycle = CustomerMotorcycleFactory(
            service_profile=self.service_profile, brand="Triumph", model="Bonneville"
        )

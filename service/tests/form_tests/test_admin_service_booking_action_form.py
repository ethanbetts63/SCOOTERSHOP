...
class ServiceBookingActionFormTest(TestCase):
    def setUp(self):
        ...
        self.service_booking_zero_payment = ServiceBookingFactory(
            payment=PaymentFactory(amount=Decimal("0.00"))
        )

    def test_valid_confirm_action_form(self):
        form = ServiceBookingActionForm(
            data={
    ...
    def test_reject_action_refund_amount_exceeds_paid_amount(self):
        ...
        self.assertEqual(
            form.errors["refund_amount"],
            [
                f"Refund amount cannot exceed the amount paid ({self.service_booking.payment.amount:.2f} {self.service_booking.payment.currency})."
            ],
        )
    ...
    def test_reject_action_zero_amount_paid_for_booking(self):
        ...
        self.assertEqual(
            form.errors["refund_amount"],
            [
                f"Refund amount cannot exceed the amount paid ({self.service_booking_zero_payment.payment.amount:.2f} {self.service_booking_zero_payment.payment.currency})."
            ],
        )

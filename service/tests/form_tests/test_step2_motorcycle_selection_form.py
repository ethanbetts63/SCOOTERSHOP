...
class MotorcycleSelectionFormTest(TestCase):
    ...
    def test_form_initialization_with_motorcycles(self):
        ...
        self.assertEqual(
            choices[0], (ADD_NEW_MOTORCYCLE_OPTION, "--- Add a New Motorcycle ---")
        )

        self.assertIn(
            (
                str(self.motorcycle1.pk),
                f"{self.motorcycle1.brand} {self.motorcycle1.model} ({self.motorcycle1.rego})",
            ),
            choices,
        )
        self.assertIn(
            (
                str(self.motorcycle2.pk),
                f"{self.motorcycle2.brand} {self.motorcycle2.model} ({self.motorcycle2.rego})",
            ),
            choices,
        )
    ...
    def test_form_initialization_without_motorcycles(self):
        ...
        self.assertEqual(
            form.fields["selected_motorcycle"].initial, ADD_NEW_MOTORCYCLE_OPTION
        )
    ...
    def test_form_valid_data_select_existing_motorcycle(self):
        ...
        self.assertEqual(
            form.cleaned_data["selected_motorcycle"], str(self.motorcycle1.pk)
        )
    ...
    def test_form_valid_data_select_add_new(self):
        ...
        self.assertTrue(form.is_valid(), f"Form is not valid: {form.errors}")
        self.assertEqual(
            form.cleaned_data["selected_motorcycle"], ADD_NEW_MOTORCYCLE_OPTION
        )
    ...
    def test_form_invalid_data_non_existent_motorcycle_id_format(self):
        ...
        self.assertIn(
            "Select a valid choice. not-an-id is not one of the available choices.",
            form.errors["selected_motorcycle"],
        )
    ...
    def test_form_invalid_data_unowned_motorcycle_id(self):
        ...
        self.assertIn(
            f"Select a valid choice. {unowned_motorcycle.pk} is not one of the available choices.",
            form.errors["selected_motorcycle"],
        )

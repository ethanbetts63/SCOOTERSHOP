...
class ServiceDetailsFormTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.active_service_type_2 = ServiceTypeFactory(
            name="Tire Replacement", is_active=True
        )
        cls.inactive_service_type = ServiceTypeFactory(
            name="Engine Rebuild", is_active=False
        )
    ...
    def test_form_valid_data(self):
        ...
        self.assertEqual(
            form.cleaned_data["service_date"], self.valid_data["service_date"]
        )
    ...
    def test_service_type_queryset(self):
        ...
        self.assertEqual(
            queryset.count(), ServiceType.objects.filter(is_active=True).count()
        )
    ...
    def test_service_date_past_date_validation(self):
        ...
        self.assertIn("service_date", form.errors)
        self.assertIn(
            "Service date cannot be in the past.", form.errors["service_date"]
        )
        ...
        self.assertTrue(
            form.is_valid(), f"Form is not valid with today's date: {form.errors}"
        )
        ...
        self.assertTrue(
            form.is_valid(), f"Form is not valid with future date: {form.errors}"
        )

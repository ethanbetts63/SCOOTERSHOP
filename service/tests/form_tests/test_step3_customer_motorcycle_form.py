...
class CustomerMotorcycleFormTest(TestCase):
    ...
    def _get_valid_data(
        self, brand_name="Honda", include_other_brand_name=False, other_brand_value=""
    ):
        valid_transmission_choice = random.choice(
            [choice[0] for choice in CustomerMotorcycle.transmission_choices]
        )
    ...
    def test_form_valid_data_new_motorcycle_other_brand_provided(self):
        ...
        data = self._get_valid_data(
            ...
            other_brand_value="MyCustomBrand",
        )
        form = CustomerMotorcycleForm(data=data)
        self.assertTrue(form.is_valid(), f"Form is not valid: {form.errors}")
    ...
    def test_form_invalid_data_other_brand_missing_name(self):
        data = self._get_valid_data(
            brand_name="Other", include_other_brand_name=True, other_brand_value=""
        )
        form = CustomerMotorcycleForm(data=data)
        self.assertFalse(form.is_valid())
        self.assertIn("other_brand_name", form.errors)
        self.assertIn(
            "Please specify the brand name when 'Other' is selected.",
            form.errors["other_brand_name"],
        )

    def test_form_valid_data_specific_brand_with_other_name_cleared(self):
        data = self._get_valid_data(
            brand_name=self.yamaha_brand.name,
            include_other_brand_name=True,
            other_brand_value="ShouldBeCleared",
        )
        form = CustomerMotorcycleForm(data=data)
        self.assertTrue(form.is_valid(), f"Form is not valid: {form.errors}")
        self.assertEqual(form.cleaned_data["other_brand_name"], "")
    ...
    def test_form_update_existing_motorcycle_to_other_brand(self):
        ...
        updated_data = self._get_valid_data(
            brand_name="Other",
            include_other_brand_name=True,
            other_brand_value="UpdatedCustomBrand",
        )
        form = CustomerMotorcycleForm(data=updated_data, instance=original_motorcycle)
        self.assertTrue(
            form.is_valid(),
            f"Form is not valid for update to other brand: {form.errors}",
        )
    ...
    def test_form_required_fields_missing(self):
        ...
        expected_required_fields = ["brand", "model", "year"]

        for field_name in expected_required_fields:
            self.assertIn(field_name, form.errors)
            self.assertTrue(
                "This field is required." in form.errors[field_name]
                or f"Motorcycle {field_name.replace('_', ' ')} is required."
                in form.errors[field_name]
            )

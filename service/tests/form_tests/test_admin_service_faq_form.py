...
class AdminServicefaqFormTest(TestCase):
    ...
    def test_form_widgets(self):
        form = AdminServicefaqForm()
        self.assertEqual(
            form.fields["booking_step"].widget.attrs["class"],
            "form-control w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500",
        )
        self.assertEqual(
            form.fields["question"].widget.attrs["class"],
            "form-control w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500",
        )
        self.assertEqual(
            form.fields["answer"].widget.attrs["class"],
            "form-control w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500",
        )
        self.assertEqual(
            form.fields["display_order"].widget.attrs["class"],
            "form-control w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500",
        )
        self.assertEqual(
            form.fields["is_active"].widget.attrs["class"],
            "form-checkbox h-5 w-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500",
        )

...
class ServiceTermsViewTest(TestCase):
    def test_service_terms_view_get_with_active_terms(self):
        active_terms = ServiceTermsFactory(
            is_active=True, content="Active Service Terms Content"
        )
    ...
    def test_service_terms_view_get_multiple_active_terms(self):
        ...
        active_terms1 = ServiceTermsFactory(
            is_active=True, content="First Active", version_number=1
        )
        active_terms2 = ServiceTermsFactory(
        ...
        self.assertEqual(
            response.context["terms"], active_terms2
        )  # Expecting the one with highest version_number

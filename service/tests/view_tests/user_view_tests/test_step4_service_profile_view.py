...
class Step4ServiceProfileViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.other_user = UserFactory(
            username="otheruser", email="other@example.com", password=cls.user_password
        )
    ...
    def test_get_auth_user_with_profile_renders_prefilled_form(self):
        self.client.force_login(self.user)
        user_profile = ServiceProfileFactory(
            user=self.user, name="Existing User Profile"
        )
    ...
    def test_post_anonymous_user_create_profile_valid_data(self):
        ...
        self.assertTrue(
            any(
                "Your details have been saved successfully." in str(m) for m in messages
            )
        )
    ...
    def test_post_auth_user_with_profile_update_profile_valid_data(self):
        ...
        existing_profile = ServiceProfileFactory(
            user=self.user, name="Old Name", email="old_email@example.com"
        )
        ...
        self.motorcycle_for_temp_booking.refresh_from_db()
        self.assertEqual(
            self.motorcycle_for_temp_booking.service_profile, existing_profile
        )
    ...
    def test_post_invalid_data_rerenders_form_with_errors(self):
        ...
        self.motorcycle_for_temp_booking.refresh_from_db()
        self.assertEqual(
            self.motorcycle_for_temp_booking.service_profile, motorcycle_initial_profile
        )
        messages = list(get_messages(response.wsgi_request))
        self.assertTrue(
            any("Please correct the errors below." in str(m) for m in messages)
        )
    ...
    def test_post_updates_motorcycle_profile_if_different(self):
        ...
        form_submitted_profile = ServiceProfile.objects.get(
            email=self.valid_post_data["email"]
        )

        self.motorcycle_for_temp_booking.refresh_from_db()

        self.assertEqual(
            self.motorcycle_for_temp_booking.service_profile, form_submitted_profile
        )
        self.assertNotEqual(
            self.motorcycle_for_temp_booking.service_profile, old_motorcycle_profile
        )

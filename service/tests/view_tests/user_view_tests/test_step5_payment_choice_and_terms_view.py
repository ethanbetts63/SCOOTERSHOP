...
from service.tests.test_helpers.model_factories import ServiceBookingFactory, ServiceProfileFactory, ServiceTypeFactory, TempServiceBookingFactory, ServiceSettingsFactory, ServiceTermsFactory, ServicefaqFactory, CustomerMotorcycleFactory, BlockedServiceDateFactory, ServiceBrandFactory
...
class Step5PaymentDropoffAndTermsViewTest(TestCase):
    ...
    def test_dispatch_no_temp_booking_uuid_in_session_redirects_to_service_home(self):
        ...
        self.assertTrue(
            any("Your booking session has expired." in str(m) for m in messages)
        )
    ...
    def test_dispatch_no_service_profile_redirects_to_step4(self):
        ...
        self.assertTrue(
            any(
                "Please complete your personal details first (Step 4)." in str(m)
                for m in messages
            )
        )
    ...
    def test_dispatch_no_service_settings_redirects_to_service_home(self):
        ...
        messages = list(get_messages(response.wsgi_request))
        self.assertTrue(
            any("Service settings are not configured." in str(m) for m in messages)
        )
    ...
    def test_dispatch_valid_temp_booking_proceeds(self):
        ...
        self.assertTemplateUsed(
            response, "service/step5_payment_dropoff_and_terms.html"
        )
    ...
    def test_get_renders_form_with_initial_data_from_temp_booking(self):
        ...
        self.assertEqual(
            form.initial["payment_method"], self.temp_booking.payment_method
        )
    ...
    def test_post_valid_data_updates_temp_booking_and_redirects_to_step6(self):
        ...
        self.assertRedirects(
            response,
            reverse("service:service_book_step6"),
            fetch_redirect_response=False,
        )

        self.temp_booking.refresh_from_db()
        self.assertEqual(
            self.temp_booking.dropoff_date,
            datetime.date.fromisoformat(self.valid_post_data["dropoff_date"]),
        )
        self.assertEqual(
            self.temp_booking.dropoff_time,
            datetime.time.fromisoformat(self.valid_post_data["dropoff_time"]),
        )
        self.assertEqual(
            self.temp_booking.payment_method, self.valid_post_data["payment_method"]
        )

        messages = list(get_messages(response.wsgi_request))
        self.assertTrue(
            any(
                "Drop-off and payment details have been saved successfully." in str(m)
                for m in messages
            )
        )
    ...
    def test_post_invalid_data_rerenders_form_with_errors(self):
        ...
        self.assertTrue(
            any(
                "Please correct the errors highlighted below." in str(m)
                for m in messages
            )
        )
    ...
    def test_post_dropoff_date_after_service_date_is_invalid(self):
        ...
        invalid_data["dropoff_date"] = (
            self.temp_booking.service_date + datetime.timedelta(days=1)
        ).strftime("%Y-%m-%d")
        ...
        self.assertIn("dropoff_date", form.errors)
        self.assertIn(
            "Drop-off date cannot be after the service date.",
            form.errors["dropoff_date"][0],
        )
    ...
    def test_post_dropoff_date_too_far_in_advance_is_invalid(self):
        ...
        invalid_data["dropoff_date"] = (
            self.temp_booking.service_date - datetime.timedelta(days=11)
        ).strftime("%Y-%m-%d")
        ...
        self.assertIn("dropoff_date", form.errors)
        self.assertIn(
            f"Drop-off cannot be scheduled more than {self.service_settings.max_advance_dropoff_days} days in advance of the service.",
            form.errors["dropoff_date"][0],
        )
    ...
    def test_post_same_day_dropoff_time_in_past_is_invalid(self):
        ...
        with self.settings(USE_TZ=True, TIME_ZONE="Australia/Perth"):
            with patch("django.utils.timezone.localtime") as mock_localtime:
                ...
                invalid_data = self.valid_post_data.copy()
                invalid_data["dropoff_date"] = datetime.date.today().strftime(
                    "%Y-%m-%d"
                )
    ...
    def test_payment_method_choices_are_correctly_populated(self):
        ...
        expected_choices = [
            (
                PAYMENT_OPTION_DEPOSIT,
                f"Pay Deposit Online (${self.service_settings.deposit_flat_fee_amount:.2f})",
            ),

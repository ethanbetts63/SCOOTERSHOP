...
class ServiceTypeDeleteViewTest(TestCase):
    ...
    def test_delete_service_type_post_success(self):
        ...
        response = self.client.post(
            reverse("service:delete_service_type", kwargs={"pk": self.service_type.pk})
        )
        ...
        self.assertEqual(
            str(messages[0]), "Service Type 'Test Service Type' deleted successfully."
        )

    def test_delete_service_type_post_nonexistent(self):
        initial_count = ServiceType.objects.count()
        response = self.client.post(
            reverse("service:delete_service_type", kwargs={"pk": 999})
        )
    ...
    def test_admin_required_mixin(self):
        ...
        self.assertRedirects(
            ...
            + "?next="
            + reverse(
                "service:delete_service_type", kwargs={"pk": self.service_type.pk}
            ),
        )

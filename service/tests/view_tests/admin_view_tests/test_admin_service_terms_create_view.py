...
from service.tests.test_helpers.model_factories import UserFactory, ServiceTermsFactory
...
class ServiceTermsCreateViewTest(TestCase):
    ...
    def test_create_service_terms_post_success(self):
        ...
        self.assertIn(f'New Service Terms Version {new_terms.version_number} created successfully and set as active.', str(messages[0]))
    ...
    def test_admin_required_mixin(self):
        ...
        response = self.client.get(reverse('service:service_terms_create'))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('service:service_terms_create'))
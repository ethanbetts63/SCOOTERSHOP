...
class ServiceTypeCreateUpdateViewTest(TestCase):
    ...
    def test_create_service_type_get(self):
        ...
        self.assertTemplateUsed(response, 'service/admin_service_type_create_update.html')
    ...
    def test_update_service_type_get(self):
        ...
        response = self.client.get(reverse('service:edit_service_type', kwargs={'pk': service_type.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'service/admin_service_type_create_update.html')
    ...
    def test_create_service_type_post_success(self):
        ...
        self.assertIn('Service Type \'New Service\' created successfully.', str(messages[0]))
    ...
    def test_update_service_type_post_success(self):
        ...
        response = self.client.post(reverse('service:edit_service_type', kwargs={'pk': service_type.pk}), data=data)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('service:service_types_management'))
        ...
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        self.assertIn('Service Type \'Updated Service\' updated successfully.', str(messages[0]))
    ...
    def test_admin_required_mixin(self):
        ...
        response = self.client.get(reverse('service:add_service_type'))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('service:add_service_type'))
...
class ServicefaqDeleteViewTest(TestCase):
    ...
    def test_service_faq_delete_get(self):
        response = self.client.get(reverse('service:service_faq_delete', kwargs={'pk': self.service_faq.pk}))
    ...
    def test_service_faq_delete_post_success(self):
        ...
        response = self.client.post(reverse('service:service_faq_delete', kwargs={'pk': self.service_faq.pk}))
        ...
        self.assertIn('The Service faq \'Test Service FAQ Question...\' was deleted successfully.', str(messages[0]))

    def test_service_faq_delete_post_nonexistent(self):
        initial_count = Servicefaq.objects.count()
        response = self.client.post(reverse('service:service_faq_delete', kwargs={'pk': 999}))
    ...
    def test_admin_required_mixin(self):
        ...
        response = self.client.get(reverse('service:service_faq_delete', kwargs={'pk': self.service_faq.pk}))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('service:service_faq_delete', kwargs={'pk': self.service_faq.pk}))
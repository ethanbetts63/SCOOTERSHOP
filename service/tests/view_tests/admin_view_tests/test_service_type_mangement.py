...
class ServiceTypeManagementViewTest(TestCase):
    ...
    def test_get_service_type_management_view(self):
        ...
        self.assertEqual(list(response.context['service_types']), [self.service_type1, self.service_type2])

    def test_admin_required_mixin(self):
        self.client.logout()
        response = self.client.get(reverse('service:service_types_management'))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('service:service_types_management'))
...
class CustomerMotorcycleDeleteViewTest(TestCase):
    ...
    def test_delete_customer_motorcycle_post_success(self):
        ...
        self.assertRedirects(
            response, reverse("service:admin_customer_motorcycle_management")
        )
        self.assertEqual(CustomerMotorcycle.objects.count(), initial_count - 1)
        self.assertFalse(
            CustomerMotorcycle.objects.filter(pk=self.customer_motorcycle.pk).exists()
        )
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        self.assertEqual(
            str(messages[0]), "Motorcycle '2020 Honda CBR' deleted successfully."
        )

    def test_delete_customer_motorcycle_post_nonexistent(self):
        initial_count = CustomerMotorcycle.objects.count()
        response = self.client.post(
            reverse("service:admin_delete_customer_motorcycle", kwargs={"pk": 999})
        )

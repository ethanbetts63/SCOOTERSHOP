...
class ServicefaqCreateUpdateViewTest(TestCase):
    ...
    def test_create_service_faq_get(self):
        ...
        self.assertTemplateUsed(response, 'service/admin_service_faq_create_update.html')
    ...
    def test_update_service_faq_get(self):
        ...
        response = self.client.get(reverse('service:service_faq_update', kwargs={'pk': faq.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'service/admin_service_faq_create_update.html')
    ...
    def test_create_service_faq_post_success(self):
        ...
        self.assertTrue(Servicefaq.objects.filter(question='New Test Question').exists())
    ...
    def test_update_service_faq_post_success(self):
        ...
        response = self.client.post(reverse('service:service_faq_update', kwargs={'pk': faq.pk}), data=data)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('service:service_faq_management'))
        faq.refresh_from_db()
    ...
    def test_admin_required_mixin(self):
        ...
        response = self.client.get(reverse('service:service_faq_create'))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('service:service_faq_create'))
...
class ServiceBrandDeleteViewTest(TestCase):
    ...
    def test_delete_service_brand_post_success(self):
        ...
        response = self.client.post(
            reverse(
                "service:delete_service_brand", kwargs={"pk": self.service_brand.pk}
            )
        )
        ...
        self.assertEqual(
            str(messages[0]), "Service brand 'Test Brand' deleted successfully."
        )

    def test_delete_service_brand_post_nonexistent(self):
        initial_count = ServiceBrand.objects.count()
        response = self.client.post(
            reverse("service:delete_service_brand", kwargs={"pk": 999})
        )

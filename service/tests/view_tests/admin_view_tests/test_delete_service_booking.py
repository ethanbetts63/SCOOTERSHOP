...
class AdminServiceBookingDeleteViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.service_booking = ServiceBookingFactory(service_booking_reference='SBK-TEST')

    def test_get_service_booking_delete_view(self):
        response = self.client.get(reverse('service:admin_delete_service_booking', kwargs={'pk': self.service_booking.pk}))
    ...
    def test_post_service_booking_delete_success(self):
        ...
        response = self.client.post(reverse('service:admin_delete_service_booking', kwargs={'pk': self.service_booking.pk}))
        ...
        self.assertFalse(ServiceBooking.objects.filter(pk=self.service_booking.pk).exists())
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        self.assertEqual(str(messages[0]), 'Booking SBK-TEST has been successfully deleted.')

    def test_post_service_booking_delete_nonexistent(self):
        initial_count = ServiceBooking.objects.count()
        response = self.client.post(reverse('service:admin_delete_service_booking', kwargs={'pk': 999}))
    ...
    def test_admin_required_mixin(self):
        ...
        response = self.client.get(reverse('service:admin_delete_service_booking', kwargs={'pk': self.service_booking.pk}))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('service:admin_delete_service_booking', kwargs={'pk': self.service_booking.pk}))
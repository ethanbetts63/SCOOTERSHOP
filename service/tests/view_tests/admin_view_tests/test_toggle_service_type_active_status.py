...
class ToggleServiceTypeActiveStatusTest(TestCase):
    ...
    def test_toggle_status_post_success_deactivate(self):
        ...
        response = self.client.post(reverse('service:toggle_service_type_active_status', kwargs={'pk': self.service_type.pk}))
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.content)
        self.assertEqual(data['status'], 'success')
        self.assertFalse(data['is_active'])
        self.assertIn('Service type \'Test Service\' has been deactivated.', data['message'])
        self.service_type.refresh_from_db()
        self.assertFalse(self.service_type.is_active)
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        self.assertEqual(str(messages[0]), 'Service type \'Test Service\' has been deactivated.')
    ...
    def test_toggle_status_post_success_activate(self):
        ...
        self.assertIn('Service type \'Test Service\' has been activated.', data['message'])
        self.service_type.refresh_from_db()
        self.assertTrue(self.service_type.is_active)
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        self.assertEqual(str(messages[0]), 'Service type \'Test Service\' has been activated.')

    def test_toggle_status_post_nonexistent_service_type(self):
        self.client.force_login(self.admin_user)
        response = self.client.post(reverse('service:toggle_service_type_active_status', kwargs={'pk': 999}))
        self.assertEqual(response.status_code, 404)

    def test_toggle_status_get_method_not_allowed(self):
        self.client.force_login(self.admin_user)
        response = self.client.get(reverse('service:toggle_service_type_active_status', kwargs={'pk': self.service_type.pk}))
    ...
    def test_toggle_status_non_staff_user(self):
        ...
        self.assertRedirects(response, reverse('users:login') + '?next=' + reverse('service:toggle_service_type_active_status', kwargs={'pk': self.service_type.pk}))

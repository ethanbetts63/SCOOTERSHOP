...
class GoogleMyBusinessAuthViewTest(TestCase):
    ...
    def test_auth_redirects_to_google(self, mock_from_client_config):
        ...
        mock_flow_instance.authorization_url.return_value = (
            "https://accounts.google.com/o/oauth2/auth?param=value",
            "test_state",
        )
        ...
        self.assertIn(
            "https://www.googleapis.com/auth/business.manage", kwargs["scopes"]
        )

        # Verify redirect_uri was set on the flow instance
        expected_redirect_uri = self.client.get(
            reverse("dashboard:gmb_callback")
        ).wsgi_request.build_absolute_uri(reverse("dashboard:gmb_callback"))
    ...
    def test_auth_redirects_if_already_configured(self):
        ...
        self.assertEqual(
            str(messages[0]),
            "You are already connected to a Google My Business account.",
        )

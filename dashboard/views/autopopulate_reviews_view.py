...
class AutopopulateReviewsView(AdminRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            data = response.json()
            print(f"AutopopulateReviewsView: Raw Google API response status: {data.get('status')}")
            print(f"AutopopulateReviewsView: Raw Google API response reviews count: {len(data.get('result', {}).get('reviews', []))}")
        ...
        if data.get("status") != "OK":
            ...
            print(f"AutopopulateReviewsView: Google Places API error status: {data.get('status')}")
            return redirect(reverse("dashboard:reviews_management"))

        reviews = data.get("result", {}).get("reviews", [])
        print(f"AutopopulateReviewsView: Total reviews from API: {len(reviews)}")

        five_star_reviews = [r for r in reviews if r.get("rating") == 5 and r.get("text")]
        print(f"AutopopulateReviewsView: 5-star reviews with text: {len(five_star_reviews)}")
        
        new_reviews_count = 0
        for review_data in five_star_reviews[:5]:
            author_name = review_data.get("author_name")
            text = review_data.get("text")
            print(f"AutopopulateReviewsView: Processing review by {author_name} (text: {text[:30]}...)")
            ...
            if not Review.objects.filter(author_name=author_name, text=text).exists():
                ...
                print(f"AutopopulateReviewsView: Review by {author_name} already exists, skipping.")

        if new_reviews_count > 0:
            messages.success(request, f"Successfully autopopulated {new_reviews_count} new 5-star reviews.")
            print(f"AutopopulateReviewsView: Successfully added {new_reviews_count} reviews.")
        else:

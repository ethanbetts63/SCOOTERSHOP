...
class GoogleMyBusinessAuthView(AdminRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if gmb_account.is_configured:
            messages.info(
                request, "You are already connected to a Google My Business account."
            )
        ...
        client_config = {
            "web": {
                ...
                "redirect_uris": [
                    request.build_absolute_uri(reverse("dashboard:gmb_callback"))
                ],
            }
        }

        flow = google_auth_oauthlib.flow.Flow.from_client_config(
            client_config, scopes=GMB_SCOPES
        )
